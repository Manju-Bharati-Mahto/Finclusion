...
const ExpenseHistoryPopup: React.FC<ExpenseHistoryPopupProps> = ({
  ...
  // Add a state object to track which months have expanded categories
  const [expandedMonths, setExpandedMonths] = useState<{[key: number]: boolean}>({});
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  
  // Reset expanded state when popup is closed
  useEffect(() => {
    if (!isOpen) {
      setExpandedMonths({});
    }
  ...
  // Set up Chart.js to prevent horizontal lines  
  useEffect(() => {
    if (ChartJS.defaults.elements?.arc) {
      ChartJS.defaults.elements.arc.borderWidth = 0;
      ChartJS.defaults.elements.arc.borderColor = 'transparent' as any;
    }
    
    // Register a plugin to ensure clean circular rendering
    ChartJS.register({
      id: 'noHorizontalLines',
      beforeDraw: (chart) => {
        const ctx = chart.ctx;
        // Apply circular clipping to hide any potential horizontal lines
        if (!ctx) return;
        ...
        // Clear canvas and apply circular clip
        ctx.save();
        ctx.beginPath();
  ...
  // Handle scroll position on resize
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Get 6 months of data for display
  ...
  const getSortedCategories = (monthIndex: number, sortType: 'value' | 'category') => {
    ...
    return categoryData; // Default sort by value
  };

  // Filter transactions for a category
  ...
  // Render transactions for selected category
  ...
  return (
    <div className="fixed inset-0 popup-container flex items-center justify-center z-50 bg-black/90 backdrop-blur-sm">
      <div className="w-[calc(100%-144px)] h-[calc(100%-144px)] max-w-[1800px] mx-auto my-auto popup-content bg-[#0a0a0a] rounded-xl border border-[#333] shadow-2xl flex flex-col relative will-change-transform">
        {showScrollTop && (
          <button 
            onClick={scrollToTop}
        ...
        <div ref={scrollContainerRef} className="overflow-y-auto custom-scrollbar flex-grow p-8 pb-12 relative scroll-smooth">
          ...
          {/* Month Cards Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 pr-2">
            {sixMonthsData.map((data, index) => (
              ...
              >
                ...
                {/* Chart Container with SVG mask overlay */}
                <div className="relative h-48 mx-auto w-full max-w-[200px] mb-4 overflow-hidden">
                  {/* Background color fill */}
                  <div className="absolute inset-0 bg-[#111] rounded-full"></div>
                  
                  {/* Chart container with mask */}
                  <div className="absolute inset-0 overflow-hidden"
                       style={{ clipPath: 'circle(45% at center)' }}>
                  ...
                  {/* SVG overlay to mask out any horizontal lines */}
                  <svg className="absolute inset-0 z-10 pointer-events-none" width="100%" height="100%" viewBox="0 0 200 200">
                    {/* Outer ring to cover any horizontal lines */}
                    <circle cx="100" cy="100" r="100" fill="#111" />
                    {/* Inner cut-out for chart visibility */}
                    <circle cx="100" cy="100" r="92" fill="none" stroke="#111" strokeWidth="1" />
                  </svg>
                </div>
                
                {/* Categories List */}
                <div className="mt-3 space-y-2 overflow-y-auto max-h-[220px] custom-scrollbar pr-1">
                  {/* Show only first 4 categories by default or all if expanded */}
                  {getSortedCategories(index, sortBy)
                    .slice(0, expandedMonths[index] ? undefined : 4)
                    .map((item, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center min-w-0 mr-2">
                            <div 
                              className="flex-shrink-0 w-4 h-4 rounded-sm mr-2"
                              style={{ backgroundColor: item.color }}
                            />
                            <span className="text-white text-sm capitalize truncate">{item.category}</span>
                          </div>
                          ...
                          <div className="flex flex-col items-end flex-shrink-0">
                            ...
                            <div className="flex items-center text-xs whitespace-nowrap">
                              <span className="text-[#999] mr-1">{categoryData.count} tx</span>
                  ...
                  {/* Toggle button for showing more/less categories */}
                  {getSortedCategories(index, sortBy).length > 4 && (
                    <button 
                      className="w-full py-2 px-3 mt-2 bg-[#222] hover:bg-[#333] text-white text-sm rounded flex items-center justify-center transition-colors"
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      {!expandedMonths[index] ? (
                        <>
                          ...
                          <span>{getSortedCategories(index, sortBy).length - 4} more categories</span>
